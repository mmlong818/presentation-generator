...
def build(slide: dict[str, Any], theme, n: int, total: int) -> list[dict]:
    ...
    for i, step in enumerate(steps):
        ...
        elements.append({
            ...
            "rotate": 0,
            "content": simple(step.get("title", ""), color=theme.text, font_size_px=sfont(theme.body), bold=True),
        ...
        if desc := step.get("desc"):
            elements.append({
                ...
                "rotate": 0,
                "content": simple(desc, color=theme.muted, font_size_px=sfont(theme.caption)),
                "defaultFontName": theme.font_body,
